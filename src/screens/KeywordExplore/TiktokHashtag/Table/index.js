...
import stylesBack from '../Content.module.sass';
...
const Table = ({
  ...
  const plushCount = 10000;
  const [activeNav, setActiveNav] = useState('post');
  const [isShowLoadMore, setIsShowLoadMore] = useState(false);
  const [startOffset, setStartOffset] = useState(0);
  const [numberPosts, setNumberPosts] = useState(10000);
  ...
  const onActionLoadMore = useCallback(() => {
    ...
    setNumberPosts(numberPosts + plushCount);
    setIsLoading(true);
  ...
  return (
    <>
        ...
        <div>
          {navigation.map((x, index) => (
            <button
              className={cn(
                styles.link,
                'text-nowrap cursor-pointer border-0 fs-mobile me-0',
                {
      ...
      <div
        className={cn(
          stylesBack.row,
          { [stylesBack.flex]: false },
          stylesBack.overflow_scroll_but_hide_scrollbar,
        )}
      >
        ...
        {!filterDataCrawled.length && !keywords.length && !isLoading && (
          <Empty
            title={'Data empty'}
            description={'Enter keyword to explore'}
          />
        )}
