...
const Table = ({
  ...
  const [limitPage, setLimitPage] = useState(10000);
  ...
  const filterDataCrawled = useMemo(() => {
    ...
    if (from) {
      result = result.filter(
        (item) =>
          stringToTimestamp(convertToDateFormat(item.created_at)) >=
          from * 1000,
      );
    }
    if (to) {
      result = result.filter(
        (item) =>
          stringToTimestamp(convertToDateFormat(item.created_at)) <= to * 1000,
      );
    }
    result = result.map((i) => {
      i.author =
  ...
  const handleSortTabPosts = useCallback(
    (orderBy) => {
      if (!orderBy) {
        return;
      }
  ...
  const handleSortTabKOCs = useCallback(
    (orderBy) => {
      if (!orderBy) {
        return;
      }
      ...
      switch (orderBy) {
        ...
        case 'views':
          ...
          break;
        case 'likes':
          setDisplayIcon(['default', 'default', `${mode}`, 'default']);
          break;
        case 'comments':
          setDisplayIcon([
            'default',
        ...
        data.reduce((acc, post) => {
          ...
          acc[authorId].latest_content =
            acc[authorId]?.latest_content < post?.created_at
              ? post?.created_at
              : acc[authorId]?.latest_content;
          acc[authorId].videos.push(post?.video);
          return acc;
        }, {}),
      );
